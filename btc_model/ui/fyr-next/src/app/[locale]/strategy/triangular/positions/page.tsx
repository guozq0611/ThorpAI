		...
		<PageWrapper>
			<div className='flex h-full w-full flex-col gap-4'>
				<div className='flex items-center justify-between'>
					<h1 className='text-2xl font-bold'>{t('三角套利策略持仓')}</h1>
						...
						<Table>
							...
							<TBody>
								{positions.map((position) => (
									<Tr key={position.id}>
										<Td className='text-left'>{position.pair1}</Td>
										<Td className='text-left'>{position.pair2}</Td>
										<Td className='text-left'>{position.pair3}</Td>
...
export default TriangularPositionsPage; 